...
TEST_CASE("Vector 4")
{
  ...
  SECTION("Lerp")
  {
    ...
    const math::vec4 dt_0 = vec4_lerp(lerp_start, lerp_end, 0.f);
    REQUIRE(math::vec4_is_near(dt_0, math::vec4_zero(), error));

    const math::vec4 dt_025 = vec4_lerp(lerp_start, lerp_end, 0.25f);
    REQUIRE(math::vec4_is_near(dt_025, math::vec4_init(0.25f, 0.25f, 0.25f, 0.25), error));

    const math::vec4 dt_05 = vec4_lerp(lerp_start, lerp_end, 0.5f);
    REQUIRE(math::vec4_is_near(dt_05, math::vec4_init(0.5f, 0.5f, 0.5f, 0.5f), error));

    const math::vec4 dt_075 = vec4_lerp(lerp_start, lerp_end, 0.75f);
    REQUIRE(math::vec4_is_near(dt_075, math::vec4_init(0.75f, 0.75f, 0.75f, 0.75f), error));

    const math::vec4 dt_1 = vec4_lerp(lerp_start, lerp_end, 1.f);
  ...
  SECTION("Normalize")
  {
    ...
    const math::vec4 normalize_vec = vec4_normalize(unnormilized_vec);
  ...
  SECTION("Dot Product")
  {
    ...
    const float dot_prod = vec4_dot(dot_vec_a, dot_vec_b);
  ...
  SECTION("Length")
  {
    ...
    const float length_of_vec = vec4_length(one_two);
